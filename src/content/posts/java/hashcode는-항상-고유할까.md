...
Java에서의 [`해시 코드`](https://bbang.dev/posts/java/object-class-tostring-equals-hashcode)는 **객체를 해시 기반 자료구조에서 효율적으로 관리하기 위해 생성되는 정수 값**이다.
...
하지만 [hashCode는 객체의 메모리 주소와 다르다](https://bbang.dev/posts/java/hashcode-메모리-주소일까). JVM 내부 구현 방식에 따라 다르겠지만, 현재는 대부분 메모리 주소와 전혀 관련이 없다.
